...
Now we will do the same using the `openalexPro` pipeline (JSON -> JSONL -> Parquet):
...
setdiff(
  snowballR$nodes$id,
  openalexSnowball::read_snowball(snowballPro, return_data = FALSE)$nodes |>
    dplyr::select(id) |>
    dplyr::mutate(
...
setdiff(
  ...
  openalexSnowball::read_snowball(snowballPro, return_data = FALSE)$edges |>
