...
## Comparison `openalexR::fetch()` and `openalexPro::pro_request()`
...
Now we will do the same using `openalexPro::pro_request()`. This involves more steps, but the result should be com[patible:
...
Works retrieved using `openalexR::oa_fetch()` and not in `openalexPro::pro_request()`:

```{r}
  setdiff(oaR$id, oaPro_comp$id)
```

Works retrieved using  `openalexPro::pro_request()` and not in `openalexR::oa_fetch()`:
...
## Comparison `openalexR::oa_snowball()` and `openalexPro::pro_snowball()`
...
Now we will do the same using `openalexPro::pro_snowball()`.
...
The following works are in `openalexR::oa_snowball()` but not in `openalexPro::pro_snowball()`:
...
```{r}
snowballPro_comp_edges <- paste0(snowballPro_comp$edges$from, " -> ", snowballPro_comp$edges$to)
snowballR_edges <- paste0(snowballR$edges$from, " -> ", snowballR$edges$to)
```

The following edges are in `openalexR::oa_snowball()` but not in `openalexPro::pro_snowball()`:
